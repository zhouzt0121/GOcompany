...
class MagnetometerEmulator {
    ...
    fun generateMagnetometerData(intensity: Float = 1.0f, timestamp: Long = System.currentTimeMillis()): SensorData {
        val time = timestamp / 1000.0
        val frequency = 2 * PI / 8.0
        ...
        return SensorData(sensorType = "MAGNETOMETER", timestamp = timestamp, values = listOf(x, y, z), accuracy = 3,
            metadata = mapOf("magnitude" to "%.2f".format(kotlin.math.sqrt(x * x + y * y + z * z))))
    }
    
    fun generateCompassHeading(heading: Float = 0.0f, variation: Float = 0.1f, timestamp: Long = System.currentTimeMillis()): SensorData {
        val time = timestamp / 1000.0
        ...
        val z = 30.0f
        return SensorData(sensorType = "MAGNETOMETER", timestamp = timestamp, values = listOf(x, y, z), accuracy = 2,
